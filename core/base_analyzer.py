...
class CodebaseAnalyzer:
    ...
    def analyze(self) -> Dict:
        ...
        if self.show_progress:
            ...
            progress = ProgressBar(total=total_items, prefix='Analyzing', enabled=True)
        else:
            progress = None
        ...
        for root, dirs, files in os.walk(self.path):
            ...
            for file in files:
                ...
                if self._is_text_file(file_path):
                    ...
                    # Hook for child classes to add custom analysis
                    if hasattr(self, '_run_advanced_analyzers'):
                        self._run_advanced_analyzers(file_path, line_count)
                
                self.file_sizes.append((file_path, file_size, language))
                
                # Update progress
                processed_files += 1
                if progress and processed_files % 10 == 0:
                    progress.update(10, suffix=f' {file_path.name[:25]}...')
        
        # Finish progress bar
        if progress:
            remaining = total_items - processed_files
            if remaining > 0:
